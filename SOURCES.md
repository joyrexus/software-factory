...
The article's richest contributions are architectural. The "map not manual" principle is implemented as an AGENTS.md file of roughly 100 lines — a table of contents pointing into a structured `docs/` directory (design docs, execution plans, product specs, references, quality scores), enabling progressive disclosure rather than context flooding. Application legibility extends beyond code: per-worktree app boot, Chrome DevTools Protocol integration for browser automation, and ephemeral local observability stacks (LogQL/PromQL) make the running application navigable by agents, not just the source. Rigid architectural layering (Types → Config → Repo → Service → Runtime → UI) is mechanically enforced as an early prerequisite, not a late luxury. Custom lint error messages inject remediation instructions directly into agent context, turning every violation into an inline coaching moment. An entropy/garbage collection pattern uses recurring background agents that scan for deviations, update quality grades, and open targeted refactoring PRs — most reviewable in under a minute and eligible for automerge. Agent-to-agent review replaces human review for most PRs: agents self-review locally, request additional agent reviews, respond to feedback, and iterate until all reviewers are satisfied. The diagnostic question driving the entire approach is not "how do we make the agent try harder?" but "what capability is missing, and how do we make it both legible and enforceable?" A preference for boring technology — composable, stable APIs well-represented in training data — leads the team to reimplement over depending on opaque libraries. Plans are treated as first-class versioned artifacts: execution plans with progress and decision logs are checked into the repository alongside active work, completed work, and known technical debt.
...
**URL:** [resources.anthropic.com/.../The-Complete-Guide-to-Building-Skill-for-Claude.pdf](https://resources.anthropic.com/hubfs/The-Complete-Guide-to-Building-Skill-for-Claude.pdf?hsLang=en) ([markdown version](https://gist.github.com/joyrexus/ff71917b4fc0a2cbc84974212da34a4a))
**Author:** Anthropic
