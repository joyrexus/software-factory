...
The shift is from collaboration to delegation. Engineers describe tasks, kick off sessions, and return to finished work. Ten tasks can run in parallel, yielding ten PRs. The agent has its own shell, IDE, and browser — it does not share the developer's environment or require their attention during execution. [Ona](../../SOURCES.md#the-self-driving-codebase) captures this role shift as moving "on the loop" — the developer sets direction and reviews outcomes rather than performing each step, maintaining oversight without occupying the execution path.
...
**Session isolation.** Each agent session runs in its own environment with no shared state between parallel runs. Session isolation extends to data: database branching services (e.g., Neon's Postgres branching) give each session an isolated database copy, so agents can write, migrate, and test against real schemas without affecting shared state ([Griffin](../../SOURCES.md#why-youre-overthinking-background-agents)). This is what makes the "ten tasks, ten PRs" model safe — sessions cannot interfere with each other, corrupt shared resources, or create race conditions. Parallelism becomes a scaling knob rather than a coordination problem.
...
VM-level isolation does not, however, cover application-layer side effects. An agent running inside a sandboxed devbox can still send real emails, post to real Slack channels, or fire real webhooks if the application code has those capabilities. Practitioners address this with environment-flag patterns — a `SANDBOX_ENV` flag that intercepts outbound calls and redirects them to a local log rather than external services ([Griffin](../../SOURCES.md#why-youre-overthinking-background-agents)). This application-layer containment is a distinct, complementary safeguard to VM isolation.
...
This double-dividend effect reinforces the [agent-native environment](../../principles/agent-native-environment.md) principle's core claim: the engineering environment matters more than agent sophistication. Organizations that invest in infrastructure quality create compounding returns — every improvement benefits an expanding population of both human and agent consumers. That said, the Stripe and Ramp approaches represent one end of a sophistication spectrum. Practitioners report that background agents are achievable with significantly less — cold-start VMs, on-demand spin-up, and roughly 500 lines of infrastructure glue — and that starting simple is a viable path that teams can iterate from as needs grow ([Griffin](../../SOURCES.md#why-youre-overthinking-background-agents)).

## Infrastructure Requirements

Beyond sandboxed execution environments, multiple sources converge on additional infrastructure requirements for effective background agents. [Ona](../../SOURCES.md#the-self-driving-codebase) organizes the requirements into three pillars — **isolated compute** (sandboxed environments where agents operate safely), **event routing** (the system mapping triggers to agent workflows), and a **governance layer** (human review gates, audit trails, bounded blast radius) — a model that aligns with the specific requirements [Pignanelli](../../SOURCES.md#agent-native-engineering) and [Dabit](../../SOURCES.md#the-cloud-agent-thesis) identify:
...
The scaling frontier is already being explored. [Cursor](../../SOURCES.md#towards-self-driving-codebases) reports multi-agent orchestration producing approximately 1,000 commits per hour — over 10 million tool calls across one week of continuous operation — to build a web browser autonomously. The architecture evolved through self-coordination, planner-executor, and finally hierarchical recursive delegation, where any agent can spawn sub-agents in a tree structure mirroring real team decomposition. Three findings from this scale are instructive: error tolerance outperforms perfect correctness (agents that recover from mistakes beat agents that try never to make them), constraints are more effective than instructions (restricting the action space works better than explaining the desired behavior), and infrastructure becomes the bottleneck before model capability does (disk I/O and compilation speed limited throughput before reasoning quality did). This last point reinforces the environment-over-agent thesis from a different angle — even with unlimited parallelism, the binding constraint is the engineering infrastructure, not the model.

Whether or not the specific timelines prove accurate, the direction is consistent with [Klaassen's](../../SOURCES.md#compound-engineering) original insight: as environments mature, agent capability compounds, and the center of gravity shifts from implementation to design.
