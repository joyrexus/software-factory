...
The gap the claims point to — tribal knowledge teams carry informally — is the hardest element to address in a spec authoring workflow. The `decisions:` field in the executable spec format is the right location for it, but the process of eliciting it (what existing patterns apply here? what are the known failure modes in this domain? what invariants does an adjacent spec impose?) is what makes a Coordinator-style analysis step genuinely valuable in practice. The agent's contribution in that step is not writing requirements — it is surfacing system context that makes human-authored requirements actually grounded.
