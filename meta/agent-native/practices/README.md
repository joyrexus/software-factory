...
The infrastructure pattern is convergent across sources. [Ona](../../../SOURCES.md#the-self-driving-codebase) identifies three pillars that background agents require: **isolated compute** (sandboxed environments where agents operate safely), **event routing** (the system mapping triggers to agent workflows), and a **governance layer** (human review gates, audit trails, bounded blast radius). Ona's framing of the developer as moving "on the loop" — setting direction and reviewing outcomes rather than performing each step — captures the same role shift that practitioner reports from Ramp, Stripe, and the General Intelligence Company describe independently.

What happens when background agents are pushed to extreme parallelism is now visible in research. [Cursor](../../../SOURCES.md#towards-self-driving-codebases) reports multi-agent orchestration producing approximately 1,000 commits per hour over a sustained week-long run, building a web browser autonomously. The architecture evolved toward hierarchical recursive delegation — agents spawning sub-agents in a tree structure mirroring real team decomposition. The key findings at this scale: error tolerance outperforms perfect correctness (agents that recover from mistakes beat agents that try never to make them), constraints are more effective than instructions, and infrastructure bottlenecks (disk I/O, compilation speed) emerge before model capability limits do.
...
The marketplace is the distribution mechanism; skills are the organizational knowledge encoded within it. [Ona](../../../SOURCES.md#organization-level-skills) frames skills as codifying "your organization's best practices into reusable skills that agents can discover and anyone can run." Two access patterns make this work: **proactive discovery** (agents automatically trigger relevant skills based on context — reviewing a PR in a team's repository activates that team's review standards) and **manual invocation** (engineers explicitly call skills by name). The concrete example is illustrative: `/review-like-mads` encodes one engineer's refined code review approach — accumulated over years — as a skill any team member can invoke. The result is that an organization's best thinking becomes available everywhere, not bottlenecked on the availability of senior engineers who developed it. This connects directly to the progressive disclosure architecture described below: proactive discovery works because frontmatter-level metadata enables routing without loading full instructions.
